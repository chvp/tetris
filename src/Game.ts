...
class Game implements IGame {
  ...
  public run = (time: number = 0): void => {
    ...
    while (this.player.dropCounter > this.player.dropInterval()) {
      ...
      this.player.dropCounter = Math.max(
        0,
        this.player.dropCounter - this.player.dropInterval(),
      );
    }
  ...
  private draw = (): void => {
    ...
    this.drawMatrix(
      this.context,
      this.player.piece.matrix,
      this.player.position,
    );
    this.drawMatrix(
      this.context,
      this.player.piece.matrix,
      this.player.fullDropPos(),
      "77",
    );
    ...
    this.drawMatrix(
      ...
      this.player.nextPiece.matrix,
      this.player.nextPiece.nextOffset,
    );
  };
